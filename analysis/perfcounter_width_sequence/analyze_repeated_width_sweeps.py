#!/usr/bin/env python3
import argparse
import re
from pathlib import Path

import numpy as np
import pandas as pd


def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--sweeps-root", required=True)
    p.add_argument("--out-dir", required=True)
    p.add_argument("--widths", default="128,256,512,1024,2048")
    p.add_argument("--reference-relpath", default="11_SP/SP_chunk001.csv")
    p.add_argument("--threshold-frac", type=float, default=0.08)
    p.add_argument("--min-active-samples", type=int, default=1)
    p.add_argument("--pad-samples", type=int, default=20)
    p.add_argument("--latest-n", type=int, default=0)
    return p.parse_args()


def parse_group_chunk(path: Path):
    group_dir = path.parent.name
    m_group = re.match(r"\d+_(.+)", group_dir)
    group = m_group.group(1) if m_group else group_dir

    m_chunk = re.search(r"_chunk(\d+)\.csv$", path.name)
    chunk = int(m_chunk.group(1)) if m_chunk else -1
    return group, chunk


def detect_regions_from_reference(ref_csv, widths, threshold_frac, min_active_samples, pad_samples):
    df = pd.read_csv(ref_csv)
    counter_cols = [c for c in df.columns if c != "elapsed_s"]

    if not counter_cols:
        raise RuntimeError(f"No counter columns in reference CSV: {ref_csv}")

    activity = df[counter_cols].abs().sum(axis=1).to_numpy(dtype=float)

    if np.max(activity) <= 0:
        raise RuntimeError(f"Reference CSV has no activity: {ref_csv}")

    threshold = np.max(activity) * threshold_frac
    active = activity > threshold

    regions = []
    in_region = False
    start = 0

    for i, is_active in enumerate(active):
        if is_active and not in_region:
            start = i
            in_region = True
        elif not is_active and in_region:
            end = i
            if end - start >= min_active_samples:
                regions.append((start, end))
            in_region = False

    if in_region:
        end = len(active)
        if end - start >= min_active_samples:
            regions.append((start, end))

    merged = []
    for s, e in regions:
        if not merged:
            merged.append((s, e))
        else:
            ps, pe = merged[-1]
            if s - pe <= 1:
                merged[-1] = (ps, e)
            else:
                merged.append((s, e))

    if len(merged) > len(widths):
        scored = []
        for s, e in merged:
            scored.append((activity[s:e].sum(), s, e))
        scored.sort(reverse=True)
        merged = sorted([(s, e) for _, s, e in scored[:len(widths)]])

    if len(merged) != len(widths):
        raise RuntimeError(
            f"Reference detection failed for {ref_csv}: "
            f"detected {len(merged)} regions, expected {len(widths)}"
        )

    windows = []
    n = len(df)

    for region_idx, ((s, e), width) in enumerate(zip(merged, widths), start=1):
        s2 = max(0, s - pad_samples)
        e2 = min(n, e + pad_samples)

        windows.append({
            "region_index": region_idx,
            "width": int(width),
            "start_idx": int(s2),
            "end_idx_exclusive": int(e2),
            "start_s": float(df["elapsed_s"].iloc[s2]),
            "end_s": float(df["elapsed_s"].iloc[e2 - 1]),
            "samples": int(e2 - s2),
        })

    return windows


def summarize_csv_with_windows(csv_path, windows, run_name):
    group, chunk = parse_group_chunk(csv_path)
    df = pd.read_csv(csv_path)

    if "elapsed_s" not in df.columns:
        return []

    counter_cols = [c for c in df.columns if c != "elapsed_s"]
    rows = []

    for w in windows:
        region = df[
            (df["elapsed_s"] >= w["start_s"]) &
            (df["elapsed_s"] <= w["end_s"])
        ]

        for counter in counter_cols:
            values = region[counter].to_numpy(dtype=float) if len(region) else np.array([])
            nonzero = values[values != 0] if len(values) else np.array([])

            rows.append({
                "run": run_name,
                "group": group,
                "chunk": chunk,
                "counter": counter,
                "width": int(w["width"]),
                "region_index": int(w["region_index"]),
                "region_start_s": float(w["start_s"]),
                "region_end_s": float(w["end_s"]),
                "samples": int(len(values)),
                "total": float(np.sum(values)) if len(values) else 0.0,
                "max": float(np.max(values)) if len(values) else 0.0,
                "mean": float(np.mean(values)) if len(values) else 0.0,
                "active_mean": float(np.mean(nonzero)) if len(nonzero) else 0.0,
                "active_samples": int(len(nonzero)),
                "csv_path": str(csv_path),
            })

    return rows


def classify_run(summary, widths):
    rows = []

    for (run, group, counter), sub in summary.groupby(["run", "group", "counter"]):
        totals = sub.groupby("width")["total"].sum().reindex(widths).fillna(0.0)

        total_all = float(totals.sum())
        t_first = float(totals.iloc[0])
        t_last = float(totals.iloc[-1])

        if total_all == 0:
            label = "zero_all_widths"
            ratio = np.nan
            corr = np.nan
            slope = 0.0
        else:
            ratio = t_last / t_first if t_first > 0 else np.inf

            x = np.array(widths, dtype=float)
            y = totals.to_numpy(dtype=float)

            corr = float(np.corrcoef(x, y)[0, 1]) if np.std(y) > 0 else 0.0
            slope = float(np.polyfit(x, y, 1)[0])

            if np.isfinite(ratio) and ratio > 8 and corr > 0.7:
                label = "strong_width_scaling"
            elif np.isfinite(ratio) and ratio > 2 and corr > 0.5:
                label = "moderate_width_scaling"
            elif np.isfinite(ratio) and ratio < 1.5:
                label = "flat_or_saturated"
            else:
                label = "active_mixed"

        row = {
            "run": run,
            "group": group,
            "counter": counter,
            "label": label,
            "total_all_widths": total_all,
            "total_width_first": t_first,
            "total_width_last": t_last,
            "scaling_ratio_last_over_first": ratio,
            "corr_total_vs_width": corr,
            "slope_total_vs_width": slope,
        }

        for width in widths:
            row[f"total_w{width}"] = float(totals.loc[width])

        rows.append(row)

    return pd.DataFrame(rows)


def cross_run_stability(classified, widths):
    rows = []

    for (group, counter), sub in classified.groupby(["group", "counter"]):
        labels = sub["label"].value_counts()
        majority_label = labels.index[0]
        majority_count = int(labels.iloc[0])
        label_consistency = majority_count / len(sub)

        ratios = sub["scaling_ratio_last_over_first"].replace([np.inf, -np.inf], np.nan).dropna()
        corrs = sub["corr_total_vs_width"].replace([np.inf, -np.inf], np.nan).dropna()

        row = {
            "group": group,
            "counter": counter,
            "n_runs": int(len(sub)),
            "majority_label": majority_label,
            "majority_label_count": majority_count,
            "label_consistency": float(label_consistency),
            "ratio_mean": float(ratios.mean()) if len(ratios) else np.nan,
            "ratio_std": float(ratios.std(ddof=1)) if len(ratios) > 1 else 0.0,
            "ratio_cv": float(ratios.std(ddof=1) / ratios.mean()) if len(ratios) > 1 and ratios.mean() != 0 else np.nan,
            "corr_mean": float(corrs.mean()) if len(corrs) else np.nan,
            "corr_std": float(corrs.std(ddof=1)) if len(corrs) > 1 else 0.0,
            "total_all_mean": float(sub["total_all_widths"].mean()),
            "total_all_std": float(sub["total_all_widths"].std(ddof=1)) if len(sub) > 1 else 0.0,
        }

        for width in widths:
            col = f"total_w{width}"
            vals = sub[col].to_numpy(dtype=float)
            row[f"{col}_mean"] = float(np.mean(vals))
            row[f"{col}_std"] = float(np.std(vals, ddof=1)) if len(vals) > 1 else 0.0
            row[f"{col}_cv"] = float(np.std(vals, ddof=1) / np.mean(vals)) if len(vals) > 1 and np.mean(vals) != 0 else np.nan

        row["stable_strong_scaling"] = (
            majority_label in ["strong_width_scaling", "moderate_width_scaling"]
            and label_consistency >= 0.67
            and (pd.isna(row["ratio_cv"]) or row["ratio_cv"] <= 0.25)
            and (pd.isna(row["corr_mean"]) or row["corr_mean"] >= 0.90)
        )

        row["near_16x_scaling"] = (
            row["ratio_mean"] >= 15.0
            and row["ratio_mean"] <= 17.0
            and row["corr_mean"] >= 0.95
        )

        rows.append(row)

    return pd.DataFrame(rows).sort_values(
        ["stable_strong_scaling", "near_16x_scaling", "total_all_mean"],
        ascending=[False, False, False],
    )


def find_sweep_dirs(root, latest_n):
    sweeps = sorted(root.glob("sweep_*"), key=lambda p: p.stat().st_mtime, reverse=True)
    if latest_n > 0:
        sweeps = sweeps[:latest_n]
    return list(reversed(sweeps))


def main():
    args = parse_args()

    sweeps_root = Path(args.sweeps_root).resolve()
    out_dir = Path(args.out_dir).resolve()
    out_dir.mkdir(parents=True, exist_ok=True)

    widths = [int(x.strip()) for x in args.widths.split(",") if x.strip()]

    sweep_dirs = find_sweep_dirs(sweeps_root, args.latest_n)

    print(f"[analysis] using {len(sweep_dirs)} sweep folders")
    for s in sweep_dirs:
        print(f"  {s}")

    all_summary = []
    all_classified = []
    all_windows = []

    for sweep_dir in sweep_dirs:
        run_name = sweep_dir.name
        ref_csv = sweep_dir / args.reference_relpath

        if not ref_csv.exists():
            print(f"[warning] skipping {run_name}: missing {ref_csv}")
            continue

        print()
        print(f"[analysis] run={run_name}")
        print(f"[analysis] reference={ref_csv}")

        try:
            windows = detect_regions_from_reference(
                ref_csv,
                widths,
                args.threshold_frac,
                args.min_active_samples,
                args.pad_samples,
            )
        except Exception as e:
            print(f"[warning] skipping {run_name}: {e}")
            continue

        for w in windows:
            all_windows.append({"run": run_name, **w})

        csvs = sorted(sweep_dir.glob("**/*_chunk*.csv"))
        print(f"[analysis] found {len(csvs)} chunk CSVs")

        rows = []
        for i, csv in enumerate(csvs, start=1):
            rows.extend(summarize_csv_with_windows(csv, windows, run_name))
            if i % 25 == 0:
                print(f"[analysis] processed {i}/{len(csvs)}")

        run_summary = pd.DataFrame(rows)
        run_classified = classify_run(run_summary, widths)

        all_summary.append(run_summary)
        all_classified.append(run_classified)

    if not all_summary:
        raise RuntimeError("No runs were successfully analyzed.")

    summary = pd.concat(all_summary, ignore_index=True)
    classified = pd.concat(all_classified, ignore_index=True)
    windows = pd.DataFrame(all_windows)
    stable = cross_run_stability(classified, widths)

    summary.to_csv(out_dir / "all_runs_counter_width_summary.csv", index=False)
    classified.to_csv(out_dir / "all_runs_counter_classification.csv", index=False)
    windows.to_csv(out_dir / "all_runs_reference_windows.csv", index=False)
    stable.to_csv(out_dir / "cross_run_stability_summary.csv", index=False)

    stable[stable["stable_strong_scaling"]].to_csv(out_dir / "stable_width_scaling_counters.csv", index=False)
    stable[stable["near_16x_scaling"]].to_csv(out_dir / "stable_near_16x_counters.csv", index=False)
    stable[stable["majority_label"] == "zero_all_widths"].to_csv(out_dir / "stable_zero_counters.csv", index=False)

    print()
    print("[analysis] wrote:")
    print(f"  {out_dir / 'all_runs_counter_width_summary.csv'}")
    print(f"  {out_dir / 'all_runs_counter_classification.csv'}")
    print(f"  {out_dir / 'all_runs_reference_windows.csv'}")
    print(f"  {out_dir / 'cross_run_stability_summary.csv'}")
    print(f"  {out_dir / 'stable_width_scaling_counters.csv'}")
    print(f"  {out_dir / 'stable_near_16x_counters.csv'}")
    print(f"  {out_dir / 'stable_zero_counters.csv'}")

    cols = [
        "group", "counter", "majority_label",
        "ratio_mean", "ratio_std", "ratio_cv",
        "corr_mean", "label_consistency",
        "total_w128_mean", "total_w2048_mean",
    ]

    print()
    print("[analysis] top stable near-16x counters:")
    print(
        stable[stable["near_16x_scaling"]]
        .sort_values("total_w2048_mean", ascending=False)
        [cols]
        .head(50)
        .to_string(index=False)
    )


if __name__ == "__main__":
    main()
