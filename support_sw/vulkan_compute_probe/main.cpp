...
#define VK_CHECK(x) do { \
    VkResult err = (x); \
    if (err != VK_SUCCESS) { \
        std::fprintf(stderr, "[vk_compute_probe] Vulkan error %d at %s:%d: %s\n", \
...
static std::vector<uint32_t> read_spv(const char* path) {
    ...
    if (!file) {
        std::fprintf(stderr, "[vk_compute_probe] Failed to open SPIR-V file: %s\n", path);
        std::exit(1);
    }

    std::streamsize size = file.tellg();
    if (size <= 0 || (size % 4) != 0) {
        std::fprintf(stderr, "[vk_compute_probe] Invalid SPIR-V size\n");
        std::exit(1);
    }
    ...
    if (!file.read(reinterpret_cast<char*>(data.data()), size)) {
        std::fprintf(stderr, "[vk_compute_probe] Failed to read SPIR-V file\n");
        std::exit(1);
    }
...
static uint32_t find_memory_type(
    ...
    std::fprintf(stderr, "[vk_compute_probe] Failed to find suitable memory type\n");
    std::exit(1);
}
...
int main(int argc, char** argv) {
    const char* spv_path = "/data/local/tmp/jerry_work/alu.comp.spv";
    uint32_t n = 1u << 18;
    ...
    std::printf("[vk_compute_probe] Starting Vulkan compute workload\n");
    std::printf("[vk_compute_probe] SPIR-V: %s\n", spv_path);
    std::printf("[vk_compute_probe] elements=%u alu_iters=%u dispatch_repeats=%u\n",
    ...
    app_info.pApplicationName = "vk_compute_probe";
    ...
    if (device_count == 0) {
        std::fprintf(stderr, "[vk_compute_probe] No Vulkan physical devices found\n");
        return 1;
    }
    ...
    std::printf("[vk_compute_probe] Using device: %s\n", props.deviceName);
    ...
    if (queue_family == UINT32_MAX) {
        std::fprintf(stderr, "[vk_compute_probe] No compute queue found\n");
        return 1;
    }

    std::printf("[vk_compute_probe] Using queue family %u, flags=0x%x\n",
    ...
    std::printf("[vk_compute_probe] Submitting workload...\n");
    VK_CHECK(vkQueueSubmit(queue, 1, &submit_info, VK_NULL_HANDLE));
    VK_CHECK(vkQueueWaitIdle(queue));
    std::printf("[vk_compute_probe] Workload complete.\n");

    std::printf("[vk_compute_probe] Verifying output...\n");

    bool ok = true;
    uint32_t check_count = std::min<uint32_t>(n, 1024);

    for (uint32_t i = 0; i < check_count; i++) {
        uint32_t expected = cpu_reference(in[i], i, alu_iters);
        if (out[i] != expected) {
            std::printf("[vk_compute_probe] MISMATCH at %u: got=0x%08x expected=0x%08x\n",
                        i, out[i], expected);
            ok = false;
            break;
        }
    ...
    if (ok) {
        std::printf("[vk_compute_probe] Verification PASSED for first %u elements.\n", check_count);
    } else {
        std::printf("[vk_compute_probe] Verification FAILED.\n");
    }
    ...
    std::printf("[vk_compute_probe] Done.\n");
    return ok ? 0 : 1;
}
