...
This repository contains ongoing research work on profiling a mobile Adreno GPU from user space on Android.
...
- KGSL tracepoint events, including command-batch lifecycle events
- Controlled workload results for ALU-heavy, memory-heavy, and copy-baseline Vulkan compute shaders

The key practical finding so far is that `kgsl_pwrstats` and RAM-wait related signals are directionally useful for distinguishing memory-heavy workloads from ALU-heavy workloads, even when direct hardware performance counters are blocked.
...
Mesa Turnip was built and modified experimentally to expose and debug A8XX raw performance-query behavior on KGSL.
...
1. Improve documentation for each attempted profiling method.
2. Add cleaner reproduction scripts for each benchmark/capture path.
3. Build a real-time visualization workflow for KGSL/sysfs/pwrstats signals.
4. Continue investigating whether Mesa Turnip or Vulkan performance-query paths can expose reliable counters.
5. Prepare method reports and figures for supervisor review / conference poster use.
