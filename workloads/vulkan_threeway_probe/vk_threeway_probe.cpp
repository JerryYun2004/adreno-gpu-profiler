...
#define VK_CHECK(x) do { \
    VkResult err = (x); \
    if (err != VK_SUCCESS) { \
        std::fprintf(stderr, "[vk_threeway_probe] Vulkan error %d at %s:%d: %s\n", \
...
static std::vector<uint32_t> read_spv(const char* path) {
    ...
    if (!file) {
        std::fprintf(stderr, "[vk_threeway_probe] Failed to open SPIR-V file: %s\n", path);
        std::exit(1);
    }

    std::streamsize size = file.tellg();
    if (size <= 0 || (size % 4) != 0) {
        std::fprintf(stderr, "[vk_threeway_probe] Invalid SPIR-V size\n");
        std::exit(1);
    }
    ...
    if (!file.read(reinterpret_cast<char*>(data.data()), size)) {
        std::fprintf(stderr, "[vk_threeway_probe] Failed to read SPIR-V file\n");
        std::exit(1);
    }
...
static uint32_t ref_alu(uint32_t input, uint32_t idx, uint32_t iters) {
...
static uint32_t ref_mem_clean(const uint32_t* in,
                              uint32_t idx,
                              uint32_t n,
                              uint32_t iters) {
    uint32_t mask = n - 1u;
    ...
    for (uint32_t i = 0; i < iters; i++) {
        ...
        uint32_t a = in[p];
        uint32_t b = in[(p + 1024u) & mask];
        uint32_t c = in[(p + 8192u) & mask];
        uint32_t d = in[(p + 65536u) & mask];
...
static uint32_t ref_copy(uint32_t input) {
    return input;
}
...
static uint32_t find_memory_type(
    ...
    std::fprintf(stderr, "[vk_threeway_probe] Failed to find suitable memory type\n");
    std::exit(1);
}
...
static Mode parse_mode(const char* s) {
    ...
    if (mode == "alu") return Mode::ALU;
    if (mode == "mem") return Mode::MEM;
    if (mode == "copy") return Mode::COPY;

    std::fprintf(stderr, "Unknown mode: %s\n", s);
    std::fprintf(stderr, "Usage: vk_threeway_probe alu|mem|copy shader.spv [n] [iters] [dispatch_repeats]\n");
    std::exit(1);
}

static const char* mode_name(Mode mode) {
    switch (mode) {
        case Mode::ALU: return "alu";
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::fprintf(stderr, "Usage: %s alu|mem|copy shader.spv [n] [iters] [dispatch_repeats]\n", argv[0]);
        return 1;
    }
    ...
    if (mode == Mode::ALU) {
        iters = 2048;
    } else if (mode == Mode::MEM) {
        iters = 512;
    } else {
        iters = 1;
    }

    if (argc >= 4) n = static_cast<uint32_t>(std::strtoul(argv[3], nullptr, 0));
    if (argc >= 5) iters = static_cast<uint32_t>(std::strtoul(argv[4], nullptr, 0));
    if (argc >= 6) dispatch_repeats = static_cast<uint32_t>(std::strtoul(argv[5], nullptr, 0));

    if ((n & (n - 1u)) != 0u) {
        std::fprintf(stderr, "[vk_threeway_probe] n must be a power of two for mem mode masking.\n");
        return 1;
    }

    std::printf("[vk_threeway_probe] mode=%s\n", mode_name(mode));
    std::printf("[vk_threeway_probe] SPIR-V: %s\n", spv_path);
    std::printf("[vk_threeway_probe] elements=%u iters=%u dispatch_repeats=%u\n",
                n, iters, dispatch_repeats);

    std::vector<uint32_t> spv = read_spv(spv_path);

    VkApplicationInfo app_info{};
    app_info.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    app_info.pApplicationName = "vk_threeway_probe";
    ...
    if (device_count == 0) {
        std::fprintf(stderr, "[vk_threeway_probe] No Vulkan physical devices found\n");
        return 1;
    }
    ...
    std::printf("[vk_threeway_probe] Using device: %s\n", props.deviceName);
    ...
    if (queue_family == UINT32_MAX) {
        std::fprintf(stderr, "[vk_threeway_probe] No compute queue found\n");
        return 1;
    }

    std::printf("[vk_threeway_probe] Using queue family %u, flags=0x%x\n",
    ...
    std::printf("[vk_threeway_probe] Submitting workload...\n");
    VK_CHECK(vkQueueSubmit(queue, 1, &submit_info, VK_NULL_HANDLE));
    VK_CHECK(vkQueueWaitIdle(queue));
    std::printf("[vk_threeway_probe] Workload complete.\n");

    std::printf("[vk_threeway_probe] Verifying output...\n");
    ...
    for (uint32_t i = 0; i < check_count; i++) {
        ...
        if (mode == Mode::ALU) {
            expected = ref_alu(in[i], i, iters);
        } else if (mode == Mode::MEM) {
            expected = ref_mem_clean(in, i, n, iters);
        } else {
            expected = ref_copy(in[i]);
        }

        if (out[i] != expected) {
            std::printf("[vk_threeway_probe] MISMATCH at %u: got=0x%08x expected=0x%08x\n",
                        i, out[i], expected);
            ok = false;
            break;
        }
    ...
    if (ok) {
        std::printf("[vk_threeway_probe] Verification PASSED for first %u elements.\n", check_count);
    } else {
        std::printf("[vk_threeway_probe] Verification FAILED.\n");
    }
    ...
    std::printf("[vk_threeway_probe] Done.\n");
